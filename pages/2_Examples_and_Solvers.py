...
with st.expander("📊 View this step by step worked example question using SCF (2 by 2)"):
    ...
    **Iteration 1: Plugging the Guess in**  
    ...
    From new calculated matrix function, we can calculate the eigenvectors for this matrix $H(x_0)$ but this would give us 2 eigenvalues and eigenvector. This is why we take the biggest eigenvalue and corresponding eigenvectors:
...
with col_mat_in1:
    base_a11 = st.number_input("Static part of $A_{11}$", value=1.0, step=0.1)
    alpha = st.slider("Nonlinearity Weight 1", 0.0, 4.0, 1.5, 0.1)
with col_mat_in2:
    off_diag = st.number_input("Diagonal ($A_{12}$ & $A_{21}$)", value=1.0, step=0.1)
with col_mat_in3:
